...
def trend_score(symbol: str):
    ...
    ema20 = ema(s,20).iloc[-1]; ema50=ema(s,50).iloc[-1]
...
def construct_candidates_no_chain(S: float, T_days: int, prefer: str, buying_power: float):
    if S<=0 or T_days<=0: return []
    r=0.0; sigma=0.35
    if prefer=="up":
...
def confidence_score(c):
    score=0; premium=c.get("mid_price",0)
...
def idea_for_symbol(symbol: str, buying_power: float):
    symbol = symbol.upper().strip()
    S = P.last_price(symbol) or 0.0
    tr = trend_score(symbol)
    prefer = tr["trend"]

    expiries = [e for e in (lambda xs=[21,28,35,42]: [dt.date.today()+dt.timedelta(days=d) for d in xs])() if 21<= (e-dt.date.today()).days <=45]

    candidates=[]
    if os.getenv("TRADIER_TOKEN"):
        for e in expiries[:2]:
            try:
                book = tradier_chain(symbol, e.isoformat())
        ...
        candidates = construct_candidates_no_chain(S, 35, prefer, buying_power)

    if not candidates:
        return {"symbol": symbol, "under_price": S, "picked_window": None, "suggestions": [],
                "explanation": "No affordable or suitable contracts found.", "thought_process": tr["notes"]}
    ...
    for c in best:
        if c["type"]=="CALL":
            plain.append(f"CALL: pay ~${c['mid_price']:.2f}. If price is above ${c['breakeven']:.2f} at expiry, you profit. Max loss is what you paid.")
        else:
            plain.append(f"PUT: pay ~${c['mid_price']:.2f}. If price is below ${c['breakeven']:.2f} at expiry, you profit. Max loss is what you paid.")

    return {
        "symbol": symbol, "under_price": S, "picked_window": None,
        "suggestions": best, "sim": sims,
        "explanation": "We target ~0.30 delta near-month, ensure it fits buying power, and align with trend.",
...
@router.get("/idea")
def idea(symbol: str, buying_power: float):
    return idea_for_symbol(symbol, buying_power)

@router.get("/market-ideas")
def market_ideas(buying_power: float, limit: int = 3):
    scored=[]
    for t in WATCHLIST:
        try:
